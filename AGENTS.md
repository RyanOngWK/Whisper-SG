...
- **PDPA redaction** (`src/infrastructure/logging/redactor.ts`): `redact()` for strings, `redactObject()` for structured data. Use `safeLog()` to emit any log entry — it scrubs personal data before any log stream. Raw transcripts must never reach the log layer unredacted. Compliant with Singapore's Personal Data Protection Act (PDPA).
...
- Column-level encryption for personal-data-bearing columns (transcripts in `conversation_turns`, metadata in `call_sessions`) — encrypted before insert via app-level KMS or pgcrypto.
